...
void tracking_module::track() {
    ...
    if (tracking_state_ == tracker_state_t::Initializing) {
        ...
        //上一步track_current_frame()是当前帧与上一阵进行跟踪，而下面这一步是通过前面获得的位姿与local map进行匹配跟踪
        //显然这样的做法可以获得更多的匹配，在更多的匹配限制之下必将会获得更准的位姿
        if (succeeded) {
            //在openvlsam中，local map表示的是，和当前frame有共视关系的那些frames和它们的landmarks
            update_local_map();
